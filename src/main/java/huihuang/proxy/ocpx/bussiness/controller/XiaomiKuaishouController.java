 ...
 **/
@RestController
@RequestMapping(Constants.ServerPath.XIAOMI_KUAISHOU)
public class XiaomiKuaishouController {

    @Autowired
    @Qualifier("xkService")
    private IChannelAdsService xkService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return xkService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public XiaomiResponse clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = xkService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
